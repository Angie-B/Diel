...
```{r}
filename <- "QC_output_wBlkSubDiel_2_Cyano_DCM_all.csv"
...
Then, the injection standards for 26R3 are super low (maybe about 1/3 of what they should be), and 28R2, 29R3, and 31R3 are all low (maybe half or 2/3 of what they should be)
...
num <- length(unique(mydata_new$Replicate.Name))-3
...
# write.csv(aq.detect,"in.cyano.aq.diel2.csv")
```

```{r combine}
measured.tqs <- aq.detect %>% filter(percent.aq.blksub>0.25) %>%
     filter(percent.aq.raw>0.1)
...
```{r, eval=FALSE}
write.csv(rsd.total, "Cyano_aq_diel2_normalization_rsd_dat.csv")
```
