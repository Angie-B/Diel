...
```{r echo=FALSE}
temp.means <- samp.dat.means %>% select(-sd )%>% spread(Compound.Name, meanArea)

colMax <- function(data) sapply(data, max, na.rm = TRUE)
temp.max <- colMax(temp.means)

temp.means.adjusted <- temp.means
for(i in 2:length(temp.max)){
     temp.means.adjusted[,i] <- temp.means[,i]/temp.max[i]
}
Adjusted.Means.Long <- temp.means.adjusted %>%
     gather(Compound.Name, AdjMean, -SampID)

temp.sd <- samp.dat.means %>% select(-meanArea )%>% spread(Compound.Name, sd)
temp.sd.adjusted <- temp.sd
for(i in 2:length(temp.max)){
     temp.sd.adjusted[,i] <- temp.sd[,i]/temp.max[i]
}
Adjusted.SD.Long <- temp.sd.adjusted %>%
