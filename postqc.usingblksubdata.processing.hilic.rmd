...
```{r echo=FALSE}
mydata.overloaded <- read_csv("HILIC/QC_output_wBlkSubDiel_1_HILIC_A_160627_allData1.akb.csv",comment = "#")
...
IS.cat <- IS.cat %>% filter(Group == "Internal Std") %>% 
     ...
     unique()
IS.list <- IS.cat[[1]]
IS.aq.list <- IS.cat[[1]][c(1:9,13,14)]
IS.aq.inj.only <- IS.cat[[1]][c(1,8)]
```
...
And so 5R1 has half the 13C-Isethionic Acid, 13C-Sulfoacetic Acid, 13C-Sulfolactic Acid, d3-Cysteic Acid, d4 Succinic Acid, Heavy Alanine, Heavy Histidine, Heavy Isoleucine
```{r correctISErrors}
